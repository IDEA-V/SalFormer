...
class SalFormer(torch.nn.Module):
    ...
    def forward(self, img, q_inputs):
        ...
        fused_features = self.cross_attention.forward(self.text_head(text_features), self.vision_head(img_features)+self.img_positional_embedding, self.vision_head(img_features)+self.img_positional_embedding, need_weights=False)[0]
        ...
        fused_features = torch.concat((img_features, self.vision_head(fused_features)), 1)
