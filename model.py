...
class SalFormer(torch.nn.Module):
    def __init__(self, vision_encoder, bert, dropout_rate=0.0):
        """
        ...
        self.v_fnn = torch.nn.Sequential(
            torch.nn.Linear(self.feature_dim, self.feature_dim),
            torch.nn.GELU(),
            torch.nn.Linear(self.feature_dim, self.feature_dim),
            torch.nn.GELU(),
        ...
        self.t_fnn = torch.nn.Sequential(
            torch.nn.Linear(self.feature_dim, self.feature_dim),
            torch.nn.GELU(),
            torch.nn.Linear(self.feature_dim, self.feature_dim),
            torch.nn.GELU(),
    ...
    def forward(self, img, q_inputs):
        ...
        img_features = self.v_fnn(img_features)

        text_features =  self.bert(**q_inputs)["last_hidden_state"]
        text_features = self.t_fnn(text_features)

        att_text_features = self.cross_attention.forward(self.text_feature_query.repeat([text_features.shape[0], 1, 1]), text_features, text_features, need_weights=False)[0]
        text_features = att_text_features + self.text_feature_query.repeat([text_features.shape[0], 1, 1])
        text_features = self.ln1(att_text_features)
