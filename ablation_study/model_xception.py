...
class SalFormer(torch.nn.Module):
    ...
    def forward(self, img, q_inputs):
        ...
        img_features = img_features.reshape([img_features.shape[0], 2048, 49]).transpose(-1, -2)
        ...
        features = self.cross_attention1.forward(self.vision_head1(img_features)+self.img_positional_embedding, fused_features, fused_features, need_weights=False)[0]
        features = self.vision_head1(img_features)+self.img_positional_embedding + features
